...
class Command:
    write, commit, sum, delete = range(4)
    ...
    for z in range(zooms[0], zooms[1] + 1):
      ...
      for y in range(0, int(ceil(float(tileData[z]['rows']) / metaData[z]['height']))):
        ...
        for x in range(0, int(ceil(float(tileData[z]['cols']) / metaData[z]['width']))):
          ...
          left   = TILE_SIZE * (int(px[z][0][0] / TILE_SIZE) +  x * metaData[z]['width'])
          top    = TILE_SIZE * (int(px[z][0][1] / TILE_SIZE) +  y * metaData[z]['height'])
          right  = left + TILE_SIZE * metawidth
